...
async function runSync({ verbose = false, useLogger = false } = {}) {
  ...
  const absoluteDataDir = path.isAbsolute(dataDir) ? dataDir : path.join(cwd, dataDir);
  ...
  try {
    ...
    try {
      log.info('Syncing budget after portfolio sync');
      await api.sync();
      log.info('Budget sync complete');
    } catch (err) {
      log.warn({ err }, 'Budget sync after portfolio sync failed');
    }
  } catch (err) {
